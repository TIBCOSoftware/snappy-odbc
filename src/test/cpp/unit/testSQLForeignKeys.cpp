    ...
    if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
      ...
      switch (UpdateRule) {
        ...
        case SQL_SET_NULL:
          ...
          break;
        case SQL_NO_ACTION:
          strcpy(szUpdateRule, "SQL_NO_ACTION");
          break;
      }
      ...
      switch (DeleteRule) {
        ...
        case SQL_SET_NULL:
          ...
          break;
        case SQL_NO_ACTION:
          strcpy(szDeleteRule, "SQL_NO_ACTION");
          break;
      }
      printf("\tColumn %d : '%s','%s','%s','%s','%s','%s','%s','%s','%s',"
          "'%s','%s','%s','%s' \n", count, szPkQualifier, szPkOwner,
          szPkTableName, szPkColumnName, szFkQualifier, szFkOwner,
          szFkTableName, szFkColumnName, szKeySeq, szUpdateRule, szDeleteRule,
          szFkName, szPkName);
    } else {
      break;
    }
...
TEST(SQLForeignKeys, FKChecks) {
  ...
  strcat(create, " (CUSTOMER_ID INTEGER, CUST_NAME CHAR(30),ADRESS CHAR(60), "
      "PHONE CHAR(15), PRIMARY KEY (CUSTOMER_ID))");
  ...
  strcat(create, " (SALES_ID INTEGER, CUSTOMER_ID INTEGER,"
      "EMPLOYEE_ID INTEGER, TOTAL_PRICE DOUBLE, ");
  strcat(create, "PRIMARY KEY (SALES_ID), ");
  strcat(create,
      "FOREIGN KEY (CUSTOMER_ID) REFERENCES " TABLE1 " (CUSTOMER_ID), ");
  strcat(create,
      "FOREIGN KEY (EMPLOYEE_ID) REFERENCES " TABLE2 " (EMPLOYEE_ID)) ");
  LOGF("\tCreate Stmt (Table3: SALES_ORDER)= '%s'", create);
  ...
  strcpy(create, "CREATE TABLE ");
  strcat(create, tabname);
  strcat(create, " (SALES_ID INTEGER, LINE_NUMBER INTEGER, PART_ID INTEGER, "
      "QUANTITY INTEGER, PRICE DOUBLE, ");
  strcat(create, "FOREIGN KEY (SALES_ID) REFERENCES " TABLE3 " (SALES_ID)) ");
  LOGF("\tCreate Stmt (Table4: SALES_LINE)= '%s'", create);
  ...
  retcode = SQLForeignKeys(hstmt, nullptr, 0, /* Primary qualifier */
      nullptr, 0, //lpSrvr->szValidLogin0, SQL_NTS, /* Primary owner     */
      (SQLCHAR*)szPkTableName, SQL_NTS, /* Primary table     */
      nullptr, 0, /* Foreign qualifier */
      nullptr, 0, /* Foreign owner     */
      nullptr, 0); /* Foreign table     */
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode,
      "SQLForeignKeys");
  ...
  /* Wenn Ownername -> "lpSrvr->szValidLogin0, SQL_NTS,"*/
  retcode = SQLForeignKeys(hstmt, nullptr, 0, /* Primary qualifier */
      nullptr, 0, /* Primary owner     */
      nullptr, 0, /* Primary table     */
      nullptr, 0, /* Foreign qualifier */
      nullptr, 0, //lpSrvr->szValidLogin0, SQL_NTS, /* Foreign owner     */
  ...
  LOGF("\tIII.) SQLForeignKeys -> with primary key tablename: "
      "'%s' and foreign key tablename: '%s'", szPkTableName, szFkTableName);
