...
RETCODE lstTableInfo(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt) {
  ...
  while (TRUE) {
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      LOGF("Row %d.: '%s','%s','%s','%s','%s'", count, szQualifier, szOwner,
          szTableName, szTableType, szRemarks);
    } else {
      break;
    }
  }
...
TEST(SQLTables, Test1) {
  ...
  /* --- Create Table --------------------------------------------- */
  SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE IF EXISTS " TABLE1, SQL_NTS);
  ...
  retcode = SQLTables(hstmt, (SQLCHAR*)szQualifier, cbQualifier, nullptr,
      cbOwner, nullptr, cbTableName, (SQLCHAR*)nullptr, cbTableType);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode, "SQLTables");
  ...
  LOGF("III.) SQLTables -> list for all types : TableTypes=%s ", szTableType);
  ...
  LOGF("IV.) SQLTables -> list for search pattern : Owner=%s, "
      "TableName=%s, TableType=%s'", szOwner, szTableName, szTableType);
...
void printTableW(const struct DataBinding* catalogResult) {
  if (catalogResult[0].StrLen_or_Ind != SQL_NULL_DATA) {
    printf("\nCatalog name  = %S\t", (wchar_t*)catalogResult[0].TargetValuePtr);
    printf("Schema name  = %S\t", (wchar_t*)catalogResult[1].TargetValuePtr);
    printf("Table name  = %S\t", (wchar_t*)catalogResult[2].TargetValuePtr);
    printf("Table type = %S\t", (wchar_t*)catalogResult[3].TargetValuePtr);
    printf("Table description = %S\n",
      (wchar_t*)catalogResult[4].TargetValuePtr);
  }
}

// remember to disconnect and free memory, and free statements and handles
TEST(SQLTables, Test2) {
  DECLARE_SQLHANDLES
  ...
  // setup the binding (can be used even if the statement is
...
void testTables(const wchar_t* catalog, const wchar_t* schema,
    const wchar_t* table, const wchar_t* tableTypes, bool expectOutput) {
  DECLARE_SQLHANDLES
  ...
  for (i = 0; i < numCols; i++) {
    ...
    catalogResult[i].BufferLength = (bufferSize + 1);
    catalogResult[i].TargetValuePtr = malloc(
      sizeof(wchar_t) * catalogResult[i].BufferLength);
  }

  retcode = SQLTablesW(hstmt, (SQLWCHAR*)catalog, catalog ? SQL_NTS : 0,
    (SQLWCHAR*)schema, schema ? SQL_NTS : 0, (SQLWCHAR*)table, table ? SQL_NTS : 0,
    (SQLWCHAR*)tableTypes, tableTypes ? SQL_NTS : 0);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode, "SQLBindCol");
...
// remember to disconnect and free memory, and free statements and handles
TEST(SQLTables, Test3) {
  printf("Running for catalog\n");
  testTables(L"%", L"", L"", L"", false);
  printf("Running for schema\n");
  testTables(L"", L"%", L"", L"", true);
  printf("Running for all nulls\n");
  testTables(0, 0, 0, L"TABLE,VIEW,SYSTEM TABLE", true);
}
