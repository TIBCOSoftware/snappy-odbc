  ...
  while (TRUE) {
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      printf("\tColumn %d : '%s','%s','%d','%s','%d','%d','%s','%s'\n", count,
          szScope, szColumnName, DataType, szTypeName, Precision, Length,
          szScale, szPseudoColumn);
    } else {
      break;
    }
  }
...
TEST(SQLSpecialColumns, CheckSpecialCols) {
  DECLARE_SQLHANDLES
  ...
  retcode = SQLSpecialColumns(hstmt, fColType, (SQLCHAR*)nullptr, 0,
      (SQLCHAR*)SCHEMA, SQL_NTS, (SQLCHAR*)TABLE1,
  ...
  retcode = SQLSpecialColumns(hstmt, fColType, (SQLCHAR*)nullptr, 0,
      (SQLCHAR*)SCHEMA, SQL_NTS, (SQLCHAR*)TABLE2,
      SQL_NTS, fScope, fNullable);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode,
      "SQLSpecialColumns");
  ...
  printf("\t fColType -> SQL_BEST_ROWID, fScope -> SQL_SCOPE_TRANSACTION ");
  fColType = SQL_BEST_ROWID;
  fScope = SQL_SCOPE_TRANSACTION;
  fNullable = SQL_NULLABLE;

  retcode = SQLSpecialColumns(hstmt, fColType, (SQLCHAR*)nullptr, 0,
      (SQLCHAR*)SCHEMA, SQL_NTS, (SQLCHAR*)TABLE3,
      SQL_NTS, fScope, fNullable);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode,
      "SQLSpecialColumns");
  ...
  printf("\t fColType -> SQL_BEST_ROWID, fScope -> SQL_SCOPE_SESSION ");
  fColType = SQL_BEST_ROWID;
  fScope = SQL_SCOPE_SESSION;
  fNullable = SQL_NULLABLE;

  retcode = SQLSpecialColumns(hstmt, fColType, (SQLCHAR*)nullptr,
