...
#define USER1 "snappydatauser1"
#define USER2 "snappydatauser2"

#define PASSWORD1 "snappydatauser1"
#define PASSWORD2 "snappydatauser2"
...
RETCODE lstColumnPrivilegesInfo(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt) {
  ...
  while (TRUE) {
    ...
    if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
      ...
      LOGF("Column %d : '%s','%s','%s','%s','%s','%s','%s','%s'", count,
          szTableQualifier, szTableOwner, szTableName, szColumnName,
...
TEST(SQLColumnPrivileges, ColumnPrivileges) {
  DECLARE_SQLHANDLES
  ...
  retcode = SQLColumnPrivileges(hstmt, nullptr, 0, /* Table qualifier */
      nullptr, SQL_NTS, /* Table owner     */
