...
TEST(SQLGetEnvAttr, Test1) {
  ...
  retcode = ::SQLGetEnvAttr(SQL_NULL_HANDLE, 1, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_INVALID_HANDLE, retcode)
      << "Expected SQL_INVALID_HANDLE from SQLGetEnvAttr";

  retcode = ::SQLGetEnvAttr(henv, 1, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_ERROR, retcode) << "Expected SQL_ERROR from SQLGetEnvAttr";

  retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_CONNECTION_POOLING, &attribute, 0,
      nullptr);
  EXPECT_EQ(SQL_SUCCESS, retcode) << "Expected SQL_SUCCESS from SQLGetEnvAttr";

  retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_CP_MATCH, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_SUCCESS, retcode) << "Expected SQL_SUCCESS from SQLGetEnvAttr";

  retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_ODBC_VERSION, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_SUCCESS, retcode)
      << "Expected SQL_SUCCESS from SQLGetEnvAttr";

  retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_OUTPUT_NTS, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_SUCCESS, retcode)
      << "Expected SQL_SUCCESS from SQLGetEnvAttr";
  ...
  EXPECT_EQ(SQL_SUCCESS, retcode)
      << "Expected SQL_SUCCESS from SQLGetEnvAttr";
  EXPECT_EQ(SQL_OV_ODBC3, attribute)
      << "ODBC version supported should be 3.0";

  retcode = ::SQLGetEnvAttr(henv, SQL_ATTR_OUTPUT_NTS, &attribute, 0, nullptr);
  EXPECT_EQ(SQL_SUCCESS, retcode)
      << "Expected SQL_SUCCESS from SQLGetEnvAttr";
  EXPECT_EQ(SQL_TRUE, (int)attribute) << "SQL_ATTR_OUTPUT_NTS should be SQL_TRUE";

  retcode = ::SQLFreeHandle(SQL_HANDLE_ENV, henv);
  EXPECT_NE(SQL_ERROR, retcode) << "SQLFreeHandle returned SQL_ERROR";
}

TEST(SQLGetEnvAttr, Test2) {
  DECLARE_SQLHANDLES

  /* ------------------------------------------------------------------------- */
  CHAR buf[MAX_NAME_LEN + 1];

  SQLINTEGER pConnectPool = 0;
  SQLINTEGER pCpMatch = 0;
  DWORD pOdbcVer;
  DWORD pOdbcVerSet;
  DWORD pOutputNts;
  ...
  LOGF(" SQL_ATTR_CONNECTION_POOLING  : '%s' ", buf);
  ...
  LOGF(" SQL_ATTR_ODBC_VERSION    : '%s' ", buf);
  ...
  LOGF(" SQL_ATTR_OUTPUT_NTS    : '%s' ", buf);
