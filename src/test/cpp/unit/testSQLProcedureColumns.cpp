...
RETCODE lstProcedureColumnsInfo(SQLHENV henv, SQLHDBC hdbc, HSTMT hstmt) {
  ...
  SQLLEN cbProcQualifier, cbProcOwner, cbProcName, cbColumnName, cbColumnType,
      cbDataType, cbTypeName, cbPrecision, cbLength, cbScale, cbRadix,
      cbNullable, cbRemarks;

  SQLSMALLINT count = 0;
  /* ---------------------------------------------------------------------har- */
  /* Bind columns in result set to storage locations */
  SQLBindCol(hstmt, 1, SQL_C_CHAR, szProcQualifier, STR_LEN,
      &cbProcQualifier);
  ...
  while (TRUE) {
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      /*
       if (cbColumnType == SQL_NULL_DATA) ColumnType=nullptr;
       if (cbNullable == SQL_NULL_DATA) Nullable=nullptr;
       */
      switch (ColumnType) {
      ...
      printf("\tProcedure %d : '%s','%s','%s','%s','%s','%s','%s','%s',"
          "'%s','%s','%s','%s','%s'\r\n", count, szProcQualifier, szProcOwner,
          szProcName, szColumnName, szColumnType, szDataType, szTypeName,
          szPrecision, szLength, szScale, szRadix, szNullable, szRemarks);
    } else {
      /* DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode,"SQLFetch"); */
      break;
    }
...
TEST(SQLProcedureColumns, BasicCheck) {
  DECLARE_SQLHANDLES
  ...
  retcode = SQLProcedureColumns(hstmt, nullptr, 0, /* Proc qualifier */
  ...
  /* ***************************************************************** */
  printf("\tII.) SQLProcedureColumns -> for USER: " USER1
      " - PROCEDURE: " PROCN "\r\n");
