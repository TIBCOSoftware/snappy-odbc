 ...
 */
TEST(SQLSetConnectAttr, BasicCheck) {
  DECLARE_SQLHANDLES

  /* ------------------------------------------------------------------------- */
  CHAR buffer[MAX_NAME_LEN + 1];

  SQLINTEGER pAccessMode, pAutoCommit, pLoginTimeout, pOptTrace = 0,
      pPacketSize, pTranslateOption, pTxnIsolation, pAutoIpd = 0, pConTimeout,
      pMetadataId, vAutoCommit, vConTimeout;
  ...
  if (pAutoCommit == SQL_AUTOCOMMIT_ON) {
    ...
    retcode = SQLSetConnectAttr(hdbc, SQL_ATTR_AUTOCOMMIT,
        (SQLPOINTER)(uintptr_t)vAutoCommit, SQL_IS_UINTEGER);
    DIAGRECCHECK(SQL_HANDLE_DBC, hdbc, 1, SQL_SUCCESS, retcode,
  ...
  if (vConTimeout > 0) {
    ...
    retcode = SQLSetConnectAttr(hdbc, SQL_ATTR_CONNECTION_TIMEOUT,
        (SQLPOINTER)(uintptr_t)vConTimeout, SQL_IS_UINTEGER);
    DIAGRECCHECK(SQL_HANDLE_DBC, hdbc, 1, SQL_SUCCESS, retcode,
