...
TEST(SQLPutData, BasicCheck) {
  ...
  strcat(create, " (NAME VARCHAR(30), AGE SMALLINT, BDATE DATE )");
  ...
  LOGF("Select Value -->\n\tName = '%s'\n\tAge = '%d'\n\tBirthday = "
      "'%d-%d-%d'\r\n", rc_sName, rc_sAge, rc_dsDate.year, rc_dsDate.month,
      rc_dsDate.day);
  ASSERT_STREQ(rc_sName, "Smith, John D.");
  ASSERT_EQ(rc_sAge, 40);
  ASSERT_EQ(rc_dsDate.year, dsDate.year);
  ASSERT_EQ(rc_dsDate.month, dsDate.month);
  ASSERT_EQ(rc_dsDate.day, dsDate.day);
