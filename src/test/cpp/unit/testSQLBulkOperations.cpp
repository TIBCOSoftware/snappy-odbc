...
TEST(SQLBulkOperations, BulkOps1) {
  DECLARE_SQLHANDLES
  ...
  /* --- Create Table . ------------------------------------------ */
  SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE IF EXISTS Customers", SQL_NTS);
  strcpy(create, "CREATE TABLE Customers (CustomerID INTEGER, "
      "CompanyName CHAR(51), Address CHAR(51), Phone CHAR(11))");
  ...
  retcode = SQLExecDirect(hstmt,
      (SQLCHAR*)"SELECT CustomerID, CompanyName, Address, "
  ...
  SQLLEN rowcount = 0;
  retcode = SQLBindCol(hstmt, 1, SQL_C_LONG, &rowcount, 0, nullptr);
  ...
  LOGF("Rowcount returned is %ld", rowcount);

  EXPECT_EQ(ROWSET_SIZE, rowcount)
      << "ROWSET_SIZE and rowcount is not matching";

  // SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE IF EXISTS Customers",
  //     SQL_NTS);
...
TEST(SQLBulkOperations, BulkOps2) {
  DECLARE_SQLHANDLES
  ...
  strcpy(create, "CREATE TABLE CUSTOMER (Cust_Num INTEGER, "
      "First_Name VARCHAR(51), Last_Name VARCHAR(51))");
  ...
  SQLCHAR sqlstmt[] = "SELECT Cust_Num, First_Name, Last_Name FROM CUSTOMER";
  ...
  SQLExecDirect(hstmt, (SQLCHAR*)"DROP TABLE IF EXISTS CUSTOMER",
      SQL_NTS);
