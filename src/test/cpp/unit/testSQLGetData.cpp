...
TEST(SQLGetData, CheckGetData) {
  ...
  strcat(create, " TYP_FIXED DECIMAL(15,2), TYP_DATE DATE, TYP_TIME TIME )");
...
TEST(SQLGetData, CheckGetData2) {
  ...
  SCHAR rgbValueDate[MAX_NAME_LEN + 1], rgbValueTime[MAX_NAME_LEN + 1];
  ...
  strcpy(create, "CREATE TABLE ");
  strcat(create, tabname);
  strcat(create, " (TYP_DATE DATE,  TYPE_TIMESTAMP TIMESTAMP )");
  LOGF("Create Stmt = '%s'", create);
  ...
  strcpy(insert, "INSERT INTO ");
  strcat(insert, tabname);
  strcat(insert, "(TYP_DATE,TYPE_TIMESTAMP)");
  strcat(insert, "VALUES ('1994-12-08',  '1994-12-08 15:45:38')");
  LOGF("Insert Stmt = '%s'", insert);
  ...
  /* --- 1. --- */
  strcpy(select, "SELECT ");
  strcat(select, "TYP_DATE,");
  strcat(select, "TYPE_TIMESTAMP");
  ...
  /* Value 1. */
  LOGF(" Get Column 1. (DATE) --> ");
  icol = 1;
  fCType = SQL_C_DATE;
  retcode = SQLGetData(hstmt, icol, fCType, rgbValueDate, sizeof(rgbValueDate),
      &pcbValue);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode, "SQLGetData");

  /* Value 2. */
  LOGF(" Get Column 2. (TIMESTAMP) --> ");
  icol = 2;
  fCType = SQL_C_TIME;
  retcode = SQLGetData(hstmt, icol, fCType, rgbValueTime, sizeof(rgbValueTime),
      &pcbValue);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode, "SQLGetData");

  retcode = SQLFreeStmt(hstmt, SQL_CLOSE);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode, "SQLFreeStmt");
  LOGF("Select Values Col.6: ->'%s'", rgbValueTime);
