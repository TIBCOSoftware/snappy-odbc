...
RETCODE lstProceduresInfo(SQLHENV henv, SQLHDBC hdbc, SQLHSTMT hstmt) {
  ...
  while (TRUE) {
    ...
    if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      /*
       if (cbProcedureType == SQL_NULL_DATA) ProcedureType=nullptr;
       */
...
TEST(SQLProcedures, CheckGet) {
  ...
  retcode = SQLProcedures(hstmt,
      nullptr, 0, (SQLCHAR*)"SYS", SQL_NTS, (SQLCHAR*)"%", SQL_NTS);
  ...
  retcode = SQLProcedures(hstmt,
      nullptr, 0, (SQLCHAR*)USER1, SQL_NTS, (SQLCHAR*)"%", SQL_NTS);
  DIAGRECCHECK(SQL_HANDLE_STMT, hstmt, 1, SQL_SUCCESS, retcode,
      "SQLProcedures");
