...
#include <string.h>
#include <climits>
#include "odbcSetup/AutoPtr.h"
#include "odbcSetup/AutoArrayPtr.h"
#include "odbcSetup/ConfigSettings.h"
...
#ifdef WIN32
  #define DriverDll "snappyodbc.dll"
  #define SetupDll  "odbcSetup.dll"
#endif
#ifdef __APPLE__ 
  #ifdef IODBC
...
int doQueryDriver()
{
  ...
  if (!SQLGetInstalledDrivers(buf, 1023, nullptr))
  {
  ...
  while (keyBuf.GetLength() < INT_MAX) // Choose 32KB as max size to ensure the loop will always terminate.
  {
    ...
    nullptr,                // nullptr means get key names in this DSN or Driver.
...
char* search_ini(char* buf, int bIsInst, int doCreate)
{
  ...
  if ((ptr = getenv (bIsInst ? "ODBCINST" : "ODBCINI")))
  {
...
int doRemoveDriver()
{
  ...
  std::string iniDriverName(pszName);
  if (!SQLWritePrivateProfileString(
    iniDriverName.c_str(),nullptr,nullptr,buf))
  {
    doPrintInstallerError( __FILE__, __LINE__ );
    fprintf( stderr, "[ERROR] Failed to remove driver.\n");
    return 0;
  ...
  std::string iniOdbcDrivers(pszOdbcDriversName);
  if (!SQLWritePrivateProfileString(
    iniOdbcDrivers.c_str(),iniDriverName.c_str(),nullptr,buf))
  {
    doPrintInstallerError( __FILE__, __LINE__ );
    fprintf( stderr, "[ERROR] Failed to remove driver.\n");
    return 0;
