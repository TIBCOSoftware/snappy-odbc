...
/// @param in_reasonForCall DllMain called with parameter telling if DLL is loaded into
///                         process or thread address space.
...
std::vector<std::string> StringSplit(const std::string &source, const char *delimiter = "|", bool keepEmpty = false)
{
    ...
    while ((next = source.find_first_of(delimiter, prev)) != std::string::npos)
    {
        ...
        {
            results.push_back(std::move(source.substr(prev, next - prev)));
        }
        prev = next + 1;
    }
    ...
    if (prev < source.size())
    {
        results.push_back(std::move(source.substr(prev)));
    }
    ...
    LPCSTR lpszArgs, LPSTR lpszMsg, WORD cbMsgMax, WORD FAR * pcbMsgOut)
{
  ...
  if (fRequest < ODBC_INSTALL_DRIVER || fRequest > ODBC_CONFIG_DRIVER_MAX)
    {
      SQLPostInstallerError (ODBC_ERROR_INVALID_REQUEST_TYPE, nullptr);
      goto done;
    }

  if (!lpszDriver || !strlen (lpszDriver))
    {
      SQLPostInstallerError (ODBC_ERROR_INVALID_NAME, nullptr);
      goto done;
    }
  ...
  switch (fRequest)
    ...
    case ODBC_INSTALL_DRIVER:
      ...
      if (!lpszArgs || !strlen (lpszArgs))
		  ...
		  out = SQLGetPrivateProfileString ("ODBC Drivers", sDesc,
			 "", driverread, sizeof (driverread), "odbcinst.ini");

			// Install driver
			SQLSetConfigMode (confMode);

			if (!SQLInstallDriverEx ("SnappyData\0Driver=snappyodbc.dll\0Setup=odbcSetup.dll\0\0", (strlen(szInPath) > 0) ? szInPath : nullptr, szOutPath,
    ...
    case ODBC_REMOVE_DRIVER:
      ...
      bResult = SQLConfigDriver(nullptr, ODBC_REMOVE_DRIVER, lpszDriver, lpszArgs, lpszMsg, cbMsgMax, pcbMsgOut );
    ...
    default:
      SQLPostInstallerError (ODBC_ERROR_REQUEST_FAILED, nullptr);
      goto done;
    };
